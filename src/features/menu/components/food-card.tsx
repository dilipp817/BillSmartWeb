...
export function FoodCard({ food, onAdd, onRemove, quantity, className }: FoodCardProps) {
  ...
  return (
    <div
      className={cn(
        "bg-card text-card-foreground flex flex-col overflow-hidden rounded-xl ring-1 transition-all select-none",
        food.is_available ? "" : "opacity-50",
        inCart ? "ring-primary/60 ring-2" : "ring-foreground/10",
        className
      )}
    >
      {/* Thumbnail + name — tapping this area navigates to food detail */}
      <Link href={`/menu/${food.id}`} className="block">
      ...
      {/* Price + quantity controls — NOT inside the nav link */}
      <div className="flex flex-1 flex-col p-3 pt-2">
        ...
        <div className="flex items-center justify-between">
          ...
          {inCart ? (
            ...
            <div className="bg-primary flex items-center gap-1 rounded-full px-1 py-0.5">
              <button
                type="button"
